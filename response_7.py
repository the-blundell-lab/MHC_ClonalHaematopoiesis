...
# Script to reproduce the figures included in response to reviewers related to the possibility of contamination with germline variants 
# This script is intended to reproduce the main figures for analysis (1-3 + SF 1-3) for MHC I WHILE EXCLUDING GERMLINE VARIANTS 
# INPUT = dataframes with binding predictions and labels (top / bottom half of binding) + list of putative germline variants
...
df = 'netmhc_2reads_rmg'

# file with scores for MHC II (required to reproduce figure 5)
...
# Data file from Hamish (06/01/2025)
germline_variants = pd.read_csv('/Users/barbarawalkowiak/Desktop/msc_thesis/data/ch_variants_with_germline_p_value.tsv', sep = '\t')

# %%
# exclude cases which are possibly germline (create dt where only cases which pass the test are retained)
germline_variants['Person_variant'] = germline_variants['Person_ID'].astype(str) + '_' + germline_variants['gene_var']
variants_germline_excluded = germline_variants[germline_variants['binom_pval']<0.05]
variants_germline_to_exclude = germline_variants[germline_variants['binom_pval']>0.05]
germline_cases = variants_germline_to_exclude['Person_variant'].tolist()
all_cases = germline_variants['Person_variant'].tolist()
somatic_cases = [x for x in all_cases if x not in germline_cases]

# %%
# which variants are sometimes classified as germline?

counts_germline = pd.DataFrame(variants_germline_to_exclude.gene_var.value_counts()).reset_index()
counts_germline['gene_var2'] = counts_germline['gene_var'].str.replace('_', '\n')
# 76 cases JAK2 (but mCA), 37 GNB1, 18 DNMT3A R882H, 12 DNMT3A R882C, 11 SRSF2 P95H, 9 SFB1 K666N, 7-1 otherwise 

ax = sns.barplot(data=counts_germline, x='gene_var2', y='count')

# Add labels and title
plt.xlabel('CH hotspot', fontsize = xaxis_font)
plt.ylabel('Number of possible germline variants', fontsize = yaxis_font)

# Add number of individuals on top of each plot 
for p in ax.patches:
    ax.text(p.get_x() + p.get_width() / 2, p.get_height() + 0.5,
            f'{p.get_height():.0f}', 
            ha='center', va='bottom', fontsize=9, color='black')

# plt.xticks(fontsize = xticks_font-4, rotation = 90)
ax = plt.gca()  # Get current axes
ax.spines['top'].set_visible(False)
ax.spines['right'].set_visible(False)
plt.xticks(rotation = 90, fontsize = xticks_font-2)
plt.savefig(f'/Users/barbarawalkowiak/Desktop/msc_thesis/results/reply/germline/4_countgermline_vars.pdf', bbox_inches='tight')

# %%
# what fraction of all variants are the putative germline ones?
counts_all = pd.DataFrame(netmhc1_df.gene_var.value_counts()).reset_index()
counts_all = counts_all.rename(columns={"count": "all_cases"})
# 76 cases JAK2 (but mCA), 37 GNB1, 18 DNMT3A R882H, 12 DNMT3A R882C, 11 SRSF2 P95H, 9 SFB1 K666N, 7-1 otherwise 

# %%
# calculate the fraction of cases for each variant that could be germline 
counts_germline = pd.merge(counts_germline, counts_all, on = 'gene_var')
counts_germline['fraction_germline'] = counts_germline['count'] / counts_germline['all_cases']

# %%
# determine which variants should be re-analysed due to possible germline contamination
order = counts_germline.groupby('gene_var2')['fraction_germline'].mean().sort_values().index[::-1]
ax = sns.barplot(data=counts_germline, x='gene_var2', y='fraction_germline', order = order)

# Add labels and title
plt.xlabel('CH hotspot', fontsize = xaxis_font)
plt.ylabel('Putative germline variants (fraction of all)', fontsize = yaxis_font)
ax = plt.gca()  # Get current axes
ax.spines['top'].set_visible(False)
ax.spines['right'].set_visible(False)
...
plt.savefig(f'/Users/barbarawalkowiak/Desktop/msc_thesis/results/reply/germline/4_countgermline_vars_fraction.pdf', bbox_inches='tight')

# %%
# Filtering for MHC I dataset
netmhc1_df_filt = netmhc1_df.copy() 
netmhc1_df_filt = netmhc1_df_filt.drop(netmhc1_df_filt.columns[0], axis=1)
netmhc1_df_filt['Person_variant'] = netmhc1_df_filt['Person_ID'].astype(str) + '_' + netmhc1_df_filt['gene_var']
netmhc1_df_filt['CH_status_rmg'] = (~netmhc1_df_filt['Person_variant'].isin(germline_cases) & ~netmhc1_df_filt['gene_var'].isna()).astype(int) # CH status = 1 if not in germline, i.e., we are sure it is somatic

# convert gene_var to na if CH_status = 0 after removing germline variants
netmhc1_df_filt['gene_var_rmg'] = np.where(netmhc1_df_filt['CH_status_rmg'] == 1, netmhc1_df_filt['gene_var'], np.nan)
print('Number of cases after excluding putative germline cases:', netmhc1_df_filt.CH_status_rmg.sum()) # 9428 cases retained

# Modify dt with labels 
netmhc1_df_labels_filt = netmhc1_df_labels.copy() 
netmhc1_df_labels_filt = netmhc1_df_labels_filt.drop(netmhc1_df_labels_filt.columns[0], axis=1)
netmhc1_df_labels_filt['Person_variant'] = netmhc1_df_labels_filt['Person_ID'].astype(str) + '_' + netmhc1_df_labels_filt['gene_var']
netmhc1_df_labels_filt['CH_status_rmg'] = (~netmhc1_df_labels_filt['Person_variant'].isin(germline_cases) & netmhc1_df_labels_filt['CH_status']==1).astype(int) # CH status = 1 if not in germline, i.e., we are sure it is somatic

# convert gene_var to na if CH_status = 0 after removing germline variants
netmhc1_df_labels_filt['gene_var_rmg'] = np.where(netmhc1_df_labels_filt['CH_status_rmg'] == 1, netmhc1_df_labels_filt['gene_var'], np.nan)
print('Number of cases after excluding putative germline cases:', netmhc1_df_labels_filt.CH_status_rmg.sum()) # 9428 cases retained

# %%
# Filtering for the MHC II dataset (for figure 5)

# convert gene_var to na if CH_status = 0 after removing germline variants
netmhc2_df_filt = netmhc2_df.copy() 
netmhc2_df_filt = netmhc2_df_filt.drop(netmhc2_df_filt.columns[0], axis=1)
netmhc2_df_filt['Person_variant'] = netmhc2_df_filt['Person_ID'].astype(str) + '_' + netmhc2_df_filt['gene_var']
netmhc2_df_filt['CH_status_rmg'] = (~netmhc2_df_filt['Person_variant'].isin(germline_cases) & ~netmhc2_df_filt['gene_var'].isna()).astype(int) # CH status = 1 if not in germline, i.e., we are sure it is somatic

# convert gene_var to na if CH_status = 0 after removing germline variants
netmhc2_df_filt['gene_var_rmg'] = np.where(netmhc2_df_filt['CH_status_rmg'] == 1, netmhc2_df_filt['gene_var'], np.nan)
print('Number of cases after excluding putative germline cases:', netmhc2_df_filt.CH_status_rmg.sum()) # 8057 cases retained

# %%
# FORMATTING AND SETTING UP THE DATAFRAME

# only use variants for which there are enough carriers (10 with MHC)
variants_identified = netmhc1_df_labels_filt.gene_var.unique().tolist() 
variants_identified = [var for var in variants_identified if netmhc1_df_labels_filt[netmhc1_df_labels_filt['gene_var'] == var].shape[0] >= 10]
variants_with_scores = netmhc1_df_labels_filt.CH_variant.unique().tolist()
...
netmhc1_df_labels_sub = netmhc1_df_labels_filt[netmhc1_df_labels_filt['CH_variant'].isin(vars_with_carriers)]
...
netmhc1_df_labels_sub['CH_status_rmg'] = netmhc1_df_labels_sub['CH_status_rmg'].astype('category') # category 

# formatting 
netmhc1_df_labels_sub['gene_var2'] = netmhc1_df_labels_sub['gene_var_rmg'].str.replace('_', '\n')
...
netmhc1_df_labels_sub['CH_status2'] = np.where(netmhc1_df_labels_sub['CH_status_rmg']==1, 'CH-positive', 'CH-negative')
...
# %%
# Subset to only include variants w min 5% individuals excluded and plot just those 
hotspots_germline = ['NRAS_G12D', 'JAK2_V617F', 'GNB1_K57E', 'SF3B1_K666N', 'SRSF2_P95L', 'KRAS_G12S', 'TP53_R273H', 'DNMT3A_R882P']
print('Variants with >= 5 possible germline cases:', hotspots_germline)
netmhc1_df_filt2 = netmhc1_df_filt[(netmhc1_df_filt['ch_status']==0) | (netmhc1_df_filt['gene_var'].isin(hotspots_germline))]
netmhc1_df_labels_sub2 = netmhc1_df_labels_sub[(netmhc1_df_labels_sub['CH_status']==0) | (netmhc1_df_labels_sub['gene_var'].isin(hotspots_germline))]
...
for var in hotspots_germline:

    # first, determine how many variants are classified as top / bottom binding in general 
    var_df = netmhc1_df_labels_sub2[netmhc1_df_labels_sub2['CH_variant']==var] # df with scores for the variant for everyone
    var_df_carriers = var_df[var_df['gene_var_rmg']==var] # df with scores for cases with variants only
...
#############################################################################
#############################################################################
# MAIN FIGURE 1b, for variants with excluded putative germline cases
...
ax = sns.stripplot(data = df_counts_carriers_melted_counts_tb[df_counts_carriers_melted_counts_tb['status']=='carriers'], 
                   dodge = True, jitter = False, x = 'gene_var2', y = 'value', hue = 'group', palette = colors, size = 5.7, edgecolor = 'black', order = order_by_total2)
...
for i, var in enumerate(order_by_total):
    ...
    # add error bars 
    plt.errorbar(x=i-0.2, y=n_carriers_top, yerr=[[std_error_top], [std_error_top]], fmt='none', capsize = 0.3, color='black', capthick=0)
    plt.errorbar(x=i+0.2, y=n_carriers_bottom, yerr=[[std_error_bottom], [std_error_bottom]], fmt='none', capsize = 0.3, color='black', capthick=0)
...
plt.xlim(-0.5, len(hotspots_germline)-0.5)
plt.ylim(0, 180)
...
plt.savefig(f'/Users/barbarawalkowiak/Desktop/msc_thesis/results/reply/germline/1b_carriers_better_worse_binding_mhci_{df}.pdf', bbox_inches='tight')

# %%
#############################################################################
#############################################################################
#############################################################################
# MAIN FIGURE 1c, for variants with excluded putative germline cases
# COMPARE PREDICTED BINDING B/N CH-POSITIVE AND CH-NEGATIVE INDIVIDUALS

# %%
# only include CH variants which have enough possible germline cases
netmhc1_df_labels_sub2 = netmhc1_df_labels_sub[netmhc1_df_labels_sub['CH_variant'].isin(hotspots_germline)]

# identify carriers 
netmhc1_df_labels_sub2_carriers = netmhc1_df_labels_sub2[netmhc1_df_labels_sub2['CH_status_rmg']==1]
...
for var in hotspots_germline:
    ...
    netmhc1_df_labels_sub_noncarriers = netmhc1_df_labels_sub2[netmhc1_df_labels_sub2['gene_var']!=var] # anyone who does not carry this variant (they may carry a different one), and not this variant as germline 
...
netmhc1_df_labels_sub_fig1c = pd.concat([netmhc1_df_labels_sub_fig1c, netmhc1_df_labels_sub2_carriers], axis = 0)

# %%
# MAIN FIGURE 1c, only for variants with excluded putative germline cases 
...
ax = sns.stripplot(y='log_score', x='CH_variant2', hue = 'CH_status2', data=netmhc1_df_labels_sub_fig1c, dodge = True, jitter = False, palette = colors, size = 2.5, legend = True, order = order_by_total2, alpha = 0.4)

# add grey background
for i in range(1, len(hotspots_germline), 2):
    ax.axvspan(i - 0.5, i + 0.5, color=col_background, alpha=0.8)
...
for i, category in enumerate(hotspots_germline):
    ...
    plt.text(i-0.35, median_carrier, '-', va='center', fontsize=40, fontweight='bold', color = col_pos)
    plt.text(i+0.11, median_noncarrier, '-', va='center', fontsize=40, fontweight='bold', color = col_neg)
...
plt.text(-0.1, -1*np.log10(0.5)+2.5, 'strong\nbinding', color='black', verticalalignment='center', fontsize = 11)
plt.text(-0.1, -1*np.log10(2)-1.5, 'no\nbinding', color='black', verticalalignment='center', fontsize = 11)
...
# specify legend 
legend = plt.legend(markerscale = 3, loc = 'upper right', fontsize = legend_font, frameon = False, handletextpad = 0.2)
for legend_handle in legend.legendHandles:
    legend_handle.set_alpha(1)
plt.savefig(f'/Users/barbarawalkowiak/Desktop/msc_thesis/results/reply/germline/1c_scores_carriers_noncarriers_{df}.pdf', bbox_inches='tight')

# %%
#############################################################################
#############################################################################
#############################################################################
# MAIN FIGURE 2 + EXTENDED DATA FIGURE 5, for variants with excluded putative germline cases
# NR OF CARRIERS IN GROUPS PREDICTED ABSOLULTE BINDING TO MHC I (SPLIT INTO TOP / BOTTOM HALF)

# find threshold variants w/ had >= 10 individuals excluded
variants_thresh = [var for var in variants_thresh if var in hotspots_germline]
variants_thresh2 = [var.replace('_', '\n') for var in variants_thresh]
...
for var in thresh_order:
    ...
    var_df_carriers = var_df[var_df['gene_var_rmg']==var] # df with scores for carriers only
...
with open(f'/Users/barbarawalkowiak/Desktop/msc_thesis/results/reply/germline/2a_carriers_mhci_contingency_tables_anybinding2_{df}.csv', "w", newline="") as f:
    w = csv.DictWriter(f, cont_tables.keys())
    w.writeheader()
    w.writerow(cont_tables)

with open(f'/Users/barbarawalkowiak/Desktop/msc_thesis/results/reply/germline/2a_carriers_mhci_p_values_anybinding2_{df}.csv', "w", newline="") as f:
    w = csv.DictWriter(f, p_values.keys())
    w.writeheader()
    w.writerow(p_values)

# %%
# MAIN FIGURE 2 results, repeated for variants with sufficient fraction of putative germline cases
...
plt.ylim(0, 0.12)
plt.savefig(f'/Users/barbarawalkowiak/Desktop/msc_thesis/results/reply/germline/2a_carriers_absolute_any_binding2_mhci_{df}_percentage.pdf', bbox_inches='tight')
...
# save dictionaries to csv
with open(f'/Users/barbarawalkowiak/Desktop/msc_thesis/results/reply/germline/2a_carriers_mhci_contingency_tables_anybinding05_{df}.csv', "w", newline="") as f:
    w = csv.DictWriter(f, cont_tables.keys())
    w.writeheader()
    w.writerow(cont_tables)

with open(f'/Users/barbarawalkowiak/Desktop/msc_thesis/results/reply/germline/2a_carriers_mhci_p_values_anybinding05_{df}.csv', "w", newline="") as f:
    w = csv.DictWriter(f, p_values.keys())
    w.writeheader()
    w.writerow(p_values)
...
plt.xlim(-0.5, len(order_by_total31)-0.5)
plt.ylim(0, 0.12)
plt.savefig(f'/Users/barbarawalkowiak/Desktop/msc_thesis/results/reply/germline/2a_carriers_absolute_any_binding_mhci_{df}_percentage.pdf', bbox_inches='tight')
...
with open(f'/Users/barbarawalkowiak/Desktop/msc_thesis/results/reply/germline/2b_carriers_mhci_contingency_tables_swbinding_{df}.csv', "w", newline="") as f:
    w = csv.DictWriter(f, cont_tables.keys())
    w.writeheader()
    w.writerow(cont_tables)

with open(f'/Users/barbarawalkowiak/Desktop/msc_thesis/results/reply/germline/2b_carriers_mhci_p_values_swbinding_{df}.csv', "w", newline="") as f:
    w = csv.DictWriter(f, p_values.keys())
    w.writeheader()
    w.writerow(p_values)
...
plt.xlim(-0.5, len(order_by_total31)-0.5)
plt.ylim(0, 0.12)
plt.savefig(f'/Users/barbarawalkowiak/Desktop/msc_thesis/results/reply/germline/2b_carriers_absolute_strong_weak_nonbinding_mhci_{df}.pdf', bbox_inches='tight')
...
for var in variants_examined:
    ...
    df_var_carriers = df_var[df_var['CH_status_rmg']==1]
    ...
    df_var_ncarriers = df_var[df_var['CH_status_rmg']==0]
...
plt.savefig(f'/Users/barbarawalkowiak/Desktop/msc_thesis/results/reply/germline/2_scores_carriers_noncarriers_percentages_mhci_{df}.pdf', bbox_inches='tight')
...
#################################################################################
# FIGURE 3
# VAF (IN CARRIERS) VS RELATIVE BINDING CAPACITY TO MHC I, repeated for variants with sufficient % of putative germline cases
...
# add legend (not on all plots though)
    ...
    plt.savefig(f'/Users/barbarawalkowiak/Desktop/msc_thesis/results/reply/germline/3a_cumulative_plot_{var}_mhci_{name}_{df}_{nr_groups}_groups_{style}.pdf', bbox_inches='tight')
...
netmhc1_df_labels_sub_carriers = netmhc1_df_labels_sub[netmhc1_df_labels_sub['CH_variant'] == netmhc1_df_labels_sub['gene_var_rmg']]
...
# one individual has DNMT3A_R882H and TP53_R175H
# the other individual has TP53_R273H and DNMT3A_R882S

#####################################################################
# SPLIT CARRIERS INTO TWO GROUPS (better vs worse binding)
netmhc1_df_labels_sub_groups2_eqsize = netmhc1_df_labels_sub_carriers.groupby('gene_var_rmg').apply(lambda x: split_into_equal_groups(x, num_groups=2))
...
netmhc1_df_labels_sub_groups3_eqsize = netmhc1_df_labels_sub_carriers.groupby('gene_var_rmg').apply(lambda x: split_into_equal_groups(x, num_groups=3))
...
# SPLIT CARRIERS INTO QUARTILES (better / mid-better / mid-worse / worse binding)
netmhc1_df_labels_sub_groups4_eqsize = netmhc1_df_labels_sub_carriers.groupby('gene_var_rmg').apply(lambda x: split_into_equal_groups(x, num_groups=4))
netmhc1_df_labels_sub_groups4_eqsize['group_name'] =  netmhc1_df_labels_sub_groups4_eqsize['group_4'].map({1: 'top', 2: 'mid-high', 3: 'mid-low', 4: 'bottom'})
netmhc1_df_labels_sub_groups4_eqsize = netmhc1_df_labels_sub_groups4_eqsize.droplevel(0)
netmhc1_df_labels_sub_groups4_eqsize = netmhc1_df_labels_sub_groups4_eqsize.reset_index()
netmhc1_df_labels_sub_groups4_eqsize_sorted = netmhc1_df_labels_sub_groups4_eqsize.sort_values(by = 'rank', ascending = False)
...
dfx_check2 = netmhc1_df_labels_sub_groups2_eqsize_sorted.groupby('gene_var_rmg').group_2.value_counts()
dfx_check3 = netmhc1_df_labels_sub_groups3_eqsize_sorted.groupby('gene_var_rmg').group_3.value_counts()
dfx_check4 = netmhc1_df_labels_sub_groups4_eqsize_sorted.groupby('gene_var_rmg').group_4.value_counts()

dfx_check2.to_csv(f'/Users/barbarawalkowiak/Desktop/msc_thesis/results/reply/germline/check_splitting_carriers_2groups.csv')
dfx_check3.to_csv(f'/Users/barbarawalkowiak/Desktop/msc_thesis/results/reply/germline/check_splitting_carriers_3groups.csv')
dfx_check4.to_csv(f'/Users/barbarawalkowiak/Desktop/msc_thesis/results/reply/germline/check_splitting_carriers_4groups.csv')
...
for var in vars_with_carriers: 
    ...
    data = netmhc1_df_labels_sub_groups2_eqsize[netmhc1_df_labels_sub_groups2_eqsize['gene_var_rmg'] == var]
    data = data.sort_values(by='VAF')
    N = data.shape[0] # number of carriers of the variant 
    
    # get VAF expressed as %
    data['VAF_percent'] = data['VAF'] * 100
...
def plot_cumulative_absolute(data, x, y, hue, colors, var, df, split, style, n1, n2, n3 = None, ks = None, legend = None):
    ...
    plt.savefig(f'/Users/barbarawalkowiak/Desktop/msc_thesis/results/reply/germline/4_VAF_threshold_{var}_{split}_{style}_{df}.pdf', bbox_inches='tight')
...
#################################################################################
#################################################################################
# FIGURE 5: MHC restriction, repeat for variants with excluded putative germline cases
...
for k in mhcrs:
    ...
    if hla[0] == 'D':
        ...
        nr_ukb = netmhc2_df_filt[hla].apply(lambda x: 0 if x == 0 else 1).sum()
        nr_carriers = netmhc2_df_filt[netmhc2_df_filt['gene_var_rmg']==gene_var][hla].apply(lambda x: 0 if x == 0 else 1).sum()
        freq_ukb = nr_ukb / len(netmhc2_df_filt.Person_ID.unique())
        freq_carriers = nr_carriers / len(netmhc2_df_filt[netmhc2_df_filt['gene_var_rmg']==gene_var].Person_ID)
        tab = [[nr_carriers, len(netmhc2_df_filt[netmhc2_df_filt['gene_var_rmg']==gene_var].Person_ID.unique())-nr_carriers], [nr_ukb, len(netmhc2_df_filt.Person_ID.unique())-nr_ukb]]
        stat, pvalue = fisher_exact(tab)

        ukb_total = len(netmhc2_df_filt.Person_ID.unique())
        carriers_total = len(netmhc2_df_filt[netmhc2_df_filt['gene_var_rmg']==gene_var].Person_ID)

        std_err_ukb = np.sqrt(freq_ukb * (1-freq_ukb)) / np.sqrt(ukb_total)
        std_err_carr = np.sqrt(freq_carriers * (1-freq_carriers)) / np.sqrt(carriers_total)
        ci_ukb = 1.96 * std_err_ukb
        ci_carr = 1.96 * std_err_carr

        values = pd.DataFrame([gene_var, hla, nr_ukb, nr_carriers, freq_ukb, freq_carriers, pvalue, std_err_ukb, std_err_carr, ci_ukb, ci_carr]).T
        values.columns = ['gene_var', 'HLA', 'nr_UKB', 'nr_CH-positive', 
                                    'freq_UKB', 'freq_CH-positive', 'pval', 'err_UKB', 'err_CH-positive','ci_UKB', 'ci_CH-positive']
        mhcr_df = pd.concat([mhcr_df, values], axis = 0)

    else:
        nr_ukb = netmhc1_df_filt[hla].apply(lambda x: 0 if x == 0 else 1).sum()
        nr_carriers = netmhc1_df_filt[netmhc1_df_filt['gene_var_rmg']==gene_var][hla].apply(lambda x: 0 if x == 0 else 1).sum()
        freq_ukb = nr_ukb / len(netmhc1_df_filt.Person_ID.unique())
        freq_carriers = nr_carriers / len(netmhc1_df_filt[netmhc1_df_filt['gene_var_rmg']==gene_var].Person_ID)
        tab = [[nr_carriers, len(netmhc1_df_filt[netmhc1_df_filt['gene_var_rmg']==gene_var].Person_ID.unique())-nr_carriers], [nr_ukb, len(netmhc1_df_filt.Person_ID.unique())-nr_ukb]]
        stat, pvalue = fisher_exact(tab)

        ukb_total = len(netmhc1_df_filt.Person_ID.unique())
        carriers_total = len(netmhc1_df_filt[netmhc1_df_filt['gene_var_rmg']==gene_var].Person_ID)
...
fig, axes = plt.subplots(1, cols, figsize=(6, 4))
...
plt.savefig('/Users/barbarawalkowiak/Desktop/msc_thesis/results/reply/germline/5a_mhc_restriction.pdf', bbox_inches='tight')
